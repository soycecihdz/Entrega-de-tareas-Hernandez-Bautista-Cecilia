...
# ╔═╡ 28655d75-d4a6-461c-9279-6bad48676220
@bind l3 Slider(0.05:0.05:2, default=0.5)

# ╔═╡ 1164f15a-d5b6-45de-90a4-70bbf4ca1705
begin 
	plot(sin, -π:l3:π, title= "Funciones trigonométricas", xlabel= "x", ylabel= "sin(x) , cos(x)", color= "purple", label= "sin(x)")
	
	plot!(cos, -π:l3:π, title= "Funciones trigonométricas", xlabel= "x", color= "pink", label= "cos(x)")
end
...
# ╔═╡ e05a8c5a-cf1e-4309-98bc-d115d31409e4


# ╔═╡ a6e29e53-c0b3-4244-9992-4ea184d8920d


# ╔═╡ 9494807c-c388-44ae-9b20-d8cd283abaf6


# ╔═╡ 2aadace3-e2f6-4290-910b-bdb981c1093e
m(x,y)= ((v*sin(θ))^2)/2g1                #Definimos la función de altura máxima
	j(x,y)= (2*(v)^2sin(θ)*cos(θ))/g1           #Definimos la función de alcance máximo
	n(x,y)= (2*v*sin(θ))/g1  
...
# ╔═╡ 77d03934-8843-4744-a99a-c77c89c691f0


# ╔═╡ c3264b4d-81b1-4e0c-9205-ff818665788c
xmax= r*cos(θ)*t1
	ymax= h+ ((r*sin(θ))^2)/(2*g)

	y= h+ (r*sin(θ))*t1-(g/2)*(t1^2)
	x= r*cos(θ)*t1
	y2= h+ (tan(θ)*x)-((g/2)/(r*cos(θ))^2)*(x^2)

	plot(plot([x,y],0:h:10, 0:θ, 2π, 0.05:r:2,0.05:t1:2))  # Tu código (comentado) va aquí :D

# ╔═╡ 01edd276-702d-4020-8894-518bad50dd5c


# ╔═╡ 57ccff9c-ecd9-4d4f-9092-588721e2b532
...
begin
	z= 2   # Definimos los valores de los parámetros.
	ñ= 1
	
	s(z) = cos(z)+sin(ñ) # Definimos la función f.
	
	@gif for q in 0:0.1:2π # para t en 0:0.1:2π
	plot(s.( range(0,2π, step = 0.1).-q),
		 legend = false, title = L"cos(x)+sin(y)")
    #= grafica Asin(B(x-t/B))+C = Asin(Bx-t)+C para todas
	las x en el rango de 0 a 2π con tamaño de paso 0.1 =#
...
# ╔═╡ a72e39a2-1d8f-4e3a-826f-7caf11d03b77


# ╔═╡ 447d1d4a-151a-4241-93b5-a6c345822ca5


# ╔═╡ 285c2e1b-d9b6-4943-9031-45ef41dd0894
...
# ╔═╡ efbd46df-35e0-4092-a8c7-268e10956b9f
@bind r Slider(0.05:0.05:2, default=0.5)

# ╔═╡ 34c3ac05-5e13-4998-82e6-d4bd375d1c45
begin
	
	g= 9.8 
	h= 5
	θ= 45
	r= 20       #Definimos r como la velocidad inicial
	t= 0:0.5:5	
	
		xmax= r*cos(θ)*t
		ymax= h+ ((r*sin(θ))^2)/(2*g)
	
		y= h+ (r*sin(θ))*t-(g/2)*(t^2)
		x= r*cos(θ)*t
		y2= h+ (tan(θ)*x)-((g/2)/(r*cos(θ))^2)*(x^2)
	
		plot([xmax,ymax], -π:l3:π, title= "Funciones trigonométricas", xlabel= "x", ylabel= "sin(x) , cos(x)", color= "purple", label= "sin(x)")
		
end

# ╔═╡ c04d41a6-ad9a-4d51-b118-f95da2dbb1fd
@bind h Slider(0.05:0.05:10, default=0.5)

# ╔═╡ 50f7f46b-081e-4b07-94af-1331b33a7c7f
begin
    g= 9.81                            #Definimos la gravedad
	                                   #Definimos la función de tiempo de vuelo
	v(g1,t1,θ)= (g1*t1)/2*sin(θ)       #Definimos la función de velocidad inicial
     w(v,θ,t1)= (v*cos(θ))t1              #Definimos la función para el eje x
p(v,θ,t1,g1)= (v*sin(θ))t1-(1/2)g1*t1^2
...
# ╔═╡ 3ea1baf9-816b-4016-a301-d22ff2bf31d0
begin
g= 9.8 
h= 5
θ= 45
...
t= 0:0.5:5	
	...
	plot([x,y2], -π:l3:π, title= "Funciones trigonométricas", xlabel= "x", ylabel= "sin(x) , cos(x)", color= "purple", label= "sin(x)")
	
end

# ╔═╡ 902e6664-b7ce-49d9-9983-2b4e3551acb7
begin
	h= 5
	θ= 45
	t= 3
	...
	g= 9.81
	x(t)= r*cos(θ)*t
	y(t)= h+ r*sin(θ)*t-(1\2)*g*t^2
		plot(x,y)
end

# ╔═╡ 2fb8bf54-63cd-4d31-978f-ee7c91878b04
@bind θ Slider(0:0.05:2π, default=0.5)

# ╔═╡ 73d1cb8f-4fc5-4a11-885d-9bc97ecd73d9
g(x,y) = [3x^2 - 3; 2y] / 50

# ╔═╡ 8912d5c3-6525-4ce5-9e2d-eac7f357da14
begin
g= 9.8 
h= 5
θ= 45
